...
const requestFunction = async function (req, res) {
    try {
        ...
        const jsonString = circularJSON.stringify(dataFromAPI);
        console.log(jsonString);
        console.log("Inside iaqi", dataFromAPI.data.iaqi);
...
// creating a function to create a database and store AQI, temperature, etc. in database
const gettingAQIOfStations = async function (req, res) {
    try {
        const apiUrl = 'https://api.waqi.info/feed/@{pathParam}/?token=7124b219cbdffcfa7e30e4e0745bc252b445fb2f';
        console.log(apiUrl);
        ...
        const csvFilePath = 'C:\\Users\\lenovo\\Downloads\\candy1.csv';
        ...
        for await (const record of readable) {
            ...
            const finalUrl = apiUrl.replace('{pathParam}', Uid);
            console.log(finalUrl);
        ...
        return res.status(201).send({ status: true, message: "API running successfully" });
    }
    catch (error) {
        return res.status(500).send({ status: false, message: error.message });
    }
...
// Creating NoSQL database using Uid
const tester3 = async function (req, res) {
    try {
        const apiUrl = 'https://api.waqi.info/feed/@{pathParam}/?token=7124b219cbdffcfa7e30e4e0745bc252b445fb2f';
        console.log(apiUrl);
        ...
        for await (const record of readable) {
            ...
            if (DominentPollutent == '') {
                if (PM10 !== "Not Available" && PM25 !== "Not Available") {
                    // DominentPollutent = "Not Available";
                    if (PM10 > PM25) {
                ...
                else if (PM10 == "Not Available" && PM25 != "Not Available") {
                    DominentPollutent = "pm25";
                }
                else if (PM10 != "Not Available" && PM25 == "Not Available") {
                    DominentPollutent = "pm10";
...
const tester4 = async function (req, res) {
    try {
...
// checking the duplicate locations
const tester5 = async function (req, res) {
    try {
        const arr = [];
        let map = new Map();

        // Displaying the file path on console
        const csvFilePath = 'C:\\Users\\lenovo\\Downloads\\aqiLocations.csv';
        console.log("File path of csv is", csvFilePath);
        ...
        console.log(arr);
        return res.status(200).send({ status: true, message: "API running fine", data: arr });
...
const geolocalisedFeed = async function (req, res) {
    try {
        ...
        const aqi = allData.data.aqi;
        const dominentPollutent = allData.data.dominentpol;
        const pm10 = allData.data.iaqi.pm10.v;
        const pm25 = allData.data.iaqi.pm25.v;
        console.log("AQI is", aqi);
        console.log("Dominent Pollutent is", dominentPollutent);
        console.log("PM10 is", pm10);
        console.log("PM25 is", pm25);

        // const createData = await requestModel.create(data);

        return res.status(200).send({ status: true, message: "Data stored in database successfully", data: allData });
    } catch (error) {
        return res.status(500).send({ status: false, message: error.message });
    }
};
